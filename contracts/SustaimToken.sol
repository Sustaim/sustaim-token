...
contract SustaimToken is ERC1155, AccessControl {
    ...
    function mint(address to, uint256 batchId, uint256 amount, uint256 projectId) external {
    ...
    function burn(address from, uint256 batchId, uint256 amount) external {
        require(from == msg.sender || hasRole(BURNER_ROLE, msg.sender), "Caller not owner or burner");
    ...
    function addProject(uint projectId, string memory name, string memory description) external {
        require(hasRole(PM_ROLE, msg.sender), "Caller not PM");
        require(bytes(_projects[projectId].name).length == 0, "Project already exists");
    ...
    function updateProjectDescription(uint projectId, string memory description) external {
        require(hasRole(PM_ROLE, msg.sender), "Caller not PM");
        require(bytes(_projects[projectId].name).length != 0, "Project doesn't exists");
        _projects[projectId] = Project(
            {
    ...
    function showProject(uint256 projectId) external view virtual returns (Project memory) {
        return _projects[projectId];
    }

    function getProjectId(uint256 batchId) external view virtual returns (uint256) {
    ...
    function getBatchAmounts(uint256 batchId) external view virtual returns (BatchAmounts memory) {
